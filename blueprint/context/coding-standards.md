...
> Your conventions. Edit these once to match your stack. Tuned for Nuxt 4 +
> Vue 3 + TypeScript; change or trim anything that doesn't fit your project.
>
> Run `/onboard` again after major stack changes. It tunes this file to the real
...
- Define interfaces/types for component props, composable returns, and data models
...
- File-based routing under `app/pages/` (create this directory when the first
  route is added; `app/app.vue` is currently the only entry point)
- Auto-imported components under `app/components/`
- Server routes under `server/api/` when a backend endpoint is needed
- Use `useFetch`/`useAsyncData` for data loading, not raw `fetch` in `onMounted`
- Use `definePageMeta` for per-page route options

## File Organization

- Components: `app/components/[feature]/ComponentName.vue`
- Pages: `app/pages/[route]/index.vue`
- Composables: `app/composables/use[Feature].ts`
- Server API routes: `server/api/[feature].ts`
...
> TODO: no database or ORM is installed yet. Update this section once the stack
> is chosen (see `project-plan.md` section 5).

## Data Fetching

- Validate all inputs with Zod (or an equivalent schema validator) once one is added
- Scope every user-owned query by the authenticated user id; never trust a
  client-supplied user id

## Error Handling

- Use try/catch in server API routes
- Return a consistent `{ success, data, error }` shape from server API responses
- Display user-friendly error messages to the user (toast or inline)
